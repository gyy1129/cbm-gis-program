...
var GeoShape = {
  ...
  transformAndDownload: function (jsonObject, resultZipFile) {
    ...
    jsonObject.features.forEach(function (item) {
      if (item.geometry) this.files[item.geometry.type].process(item)
        ...
        //GeometryCollection
        item.geometries.forEach(function (item) {
          var gitem = {}
          gitem.geometry = item
    ...
    zip.generateAsync({ type: 'blob' }).then(
      ...
      function (err) {
        document.getElementById('blob').innerText(err)
      }
    )
  },
  /**
   ...
   * @param {any} rec - ShapeRecord object
   */
  _recGetPointContentLength: function () {
    return 20
  },
  ...
  _recWritePolygonGeometry: function (rec, shpView, offset, id) {
    ...
    ofs = 8 + 44 + offset
    for (let i = 0; i < rec.partCount; i++) shpView.setInt32(ofs + 4 * i, rec.partIndices[i], true)
    ofs = 8 + 44 + offset + 4 * rec.partCount
    for (let i = 0; i < pc; i++) shpView.setFloat64(ofs + 8 * i, rec.points[i], true)
  },
  _recWriteMultiPointGeometry: function (rec, shpView, offset, id) {
    ...
    ofs = 8 + 40 + offset
    for (var i = 0; i < pc; i++) shpView.setFloat64(ofs + 8 * i, rec.points[i], true)
  }
}
...
class ESRIFileGen {
  ...
  writeDbf() {
    ...
    for (var item in this.propertyNames) {
      ...
      for (var i = 0; i < item.length; i++) f[i] = item.charCodeAt(i)
      fieldCount++
      recordBytes += flen
      h = h.concat(f)
    }
...
class MultiPolygonFileGen extends ESRIFileGen {
  ...
  process(item) {
    var g = item.geometry.coordinates
    var rec = new ShapeRecord(this, item)
    g.forEach(function (mpg) {
      mpg.forEach(function (pg) {
        rec.partIndices[rec.partCount++] = rec.points.length / 2
        pg.forEach(function (item) {
          rec.checkBounds(item[0], item[1])
          rec.points.push(item[0], item[1])
